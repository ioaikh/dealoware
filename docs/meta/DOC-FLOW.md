...
**Product alignment:** Product goals/facts in `product/PRODUCT-BRIEF.md` are source of truth. Do not invent Stories or requirements.
...
3. **Product goals/facts** (`product/PRODUCT-BRIEF.md`) constrain all artifacts. Conflicts → note in doc + escalate **PM → Product → CEO**.
...
|---------------|------------|-------------|----------------|----------------------|---------|
| Product goals / domain / claims | Product Team | `product/` | Prefer stable names for seeds (`PRODUCT-BRIEF.md`); dated files use pattern below | On create/change (hourly delta) | PM assign → Chief Docs brief → Senior Docs → Docs QA |
...
### Allowed `{team}` enum
`sa` | `spec` | `devplan` | `sd` | `qa` | `security` | `devops` | `ba` | `pm` | `product` | `business` | `docs` | `core` | `agents`
...
Seed/charter files may keep stable undated names (`PRODUCT-BRIEF.md`, `ORG-OPS.md`, `DOC-FLOW.md`).
...
1. Docs **must not invent** Product Stories or requirements beyond `product/PRODUCT-BRIEF.md`.
...
- [ ] Aligns to Product brief; no invented Stories
